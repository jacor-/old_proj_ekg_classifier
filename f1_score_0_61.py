...
Y_train_def_2 = [1 if x =='V' else 0 for x in Y_train]
Y_valid_def_2 = [1 if x =='V' else 0 for x in Y_valid]

clf = KNeighborsClassifier(n_neighbors = 10)
clf.fit(pa2.project(pa.project(numpy.matrix(X_train,dtype=numpy.float64))), Y_train_def_2)
pred = clf.predict(pa2.project(pa.project(numpy.matrix(X_valid, dtype=numpy.float64))))
print classification_report(pred, Y_valid_def_2)
