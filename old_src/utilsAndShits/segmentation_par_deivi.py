...
class DOG1(Cwt):
    ...
    def wf(self, s_omega):
        from numpy import complex64, sqrt, pi
...
def QRSon(A):
    ...
    M = argmax(ss)
    aux = 0
    while not( ss[M-2-aux]<ss[M-1-aux] and ss[M-1-aux]>ss[M-aux]   ):
        aux = aux+1
...
def QRSoff(A):
    ...
    M = argmax(ss)
    aux1 = 0
    while not( ss[M+2+aux1]<ss[M+1+aux1] and ss[M+1+aux1]>ss[M+aux1]   ):
        aux1 = aux1+1    
    
    return M+aux1


qrson = [abs(QRSon(Ecg.objects()[i].median['HES']['V1'])) for i in range(10)]
qrsoff = [abs(QRSoff(Ecg.objects()[i].median['HES']['V1'])) for i in range(10)]
